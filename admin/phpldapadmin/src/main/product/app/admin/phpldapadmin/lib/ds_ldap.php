	...
	private function unescapeDN($dn) {
		...
		if (is_array($dn)) {
			...
			foreach ($dn as $key => $rdn)
				$a[$key] = preg_replace_callback('/\\\([0-9A-Fa-f]{2})/',array($this, 'unescapeDN_callback'),$rdn);

			return $a;

		} else
			return preg_replace_callback('/\\\([0-9A-Fa-f]{2})/',array($this, 'unescapeDN_callback'),$dn);
	}

	/**
	 * Callback for preg_replace_callback in unescapeDN (Project Factory PHP 5.5 support)
	 */
	private function unescapeDN_callback($matches) {
