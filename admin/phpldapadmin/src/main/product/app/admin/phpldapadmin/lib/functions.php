 ...
 */
function password_hash_ldap($password_clear,$enc_type) { // Project Factory PHP 5.5 support
	if (DEBUG_ENABLED && (($fargs=func_get_args())||$fargs='NOARGS'))
		debug_log('Entered (%%)',1,0,__FILE__,__LINE__,__METHOD__,$fargs);
...
function password_check($cryptedpassword,$plainpassword,$attribute='userpassword') {
	...
	switch($cypher) {
		...
		case 'sha':
			if (strcasecmp(password_hash_ldap($plainpassword,'sha'),'{SHA}'.$cryptedpassword) == 0) // Project Factory PHP 5.5 support
				return true;
			else
				return false;
		...
		case 'md5':
			if( strcasecmp(password_hash_ldap($plainpassword,'md5'),'{MD5}'.$cryptedpassword) == 0) // Project Factory PHP 5.5 support
				return true;
			else
				return false;
		...
		case 'sha512':
			if (strcasecmp(password_hash_ldap($plainpassword,'sha512'),'{SHA512}'.$cryptedpassword) == 0) // Project Factory PHP 5.5 support
				return true;
			else
				return false;
...
function dn_unescape($dn) {
	...
	if (is_array($dn)) {
		...
		foreach ($dn as $key => $rdn)
			$a[$key] = preg_replace_callback('/\\\([0-9A-Fa-f]{2})/','dn_unescape_callback',$rdn);

		return $a;

	} else {
		return preg_replace_callback('/\\\([0-9A-Fa-f]{2})/','dn_unescape_callback',$dn);
	}
}

/**
 * Callback for preg_replace_callback in dn_unescape (Project Factory PHP 5.5 support)
 */
function dn_unescape_callback($matches) {
