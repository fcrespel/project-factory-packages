...
################################################################################
# Modified by Fabien CRESPEL <fabien@crespel.net>
# - Allow '-' and '_' characters in group name (see simplify_name)
# - Copy final file instead of renaming it, to avoid error on different FS
################################################################################

import ConfigParser, datetime, getpass, os, re, sys, tempfile, shutil # F. CRESPEL edit: add shutil
...
application_version = "1.0.3" # F. CRESPEL note: this is the original version
...
def print_group_model(groups, memberships):
  ...
  if authz_path:
    ...
    shutil.copyfile(tmp_authz_path, authz_path) # F. CRESPEL edit: copy file instead of renaming it
    os.remove(tmp_authz_path) # F. CRESPEL edit: remove temporary file after the copy
  else:
    tmpfile = open(tmp_authz_path, 'r')
