...
chown -R @{httpd.user}:@{httpd.group} "@{package.data}" "@{package.log}"
...
if [ "$DO_DBINIT" -eq 1 ]; then
	...
	if [ -n "$TESTLINK_DB_VERSION" ]; then
		find "@{package.app}/install/sql/alter_tables" -wholename '*/mysql/DB.*' -name '*.sql' -printf '%%P\n' | sort -t '/' -k 3,3 -k 4,4 -V | while read FILENAME; do
			INSTALL_DB_VERSION=`echo "$FILENAME" | cut -d'/' -f 3`
			if [ "$INSTALL_DB_VERSION" != "`echo -e "$TESTLINK_DB_VERSION\n$INSTALL_DB_VERSION" | sort -V | head -n1`" ]; then
				if ! cat "@{package.app}/install/sql/alter_tables/$FILENAME" | mysql_exec "@{testlink.db.name}"; then
