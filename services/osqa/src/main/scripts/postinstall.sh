...
if ! easy_install -H None -f "$PYTHON_EGGS_DIR" $PYTHON_MODULES > "$POSTINSTALL_OUTPUT" 2>&1; then
...
if [ "$DO_DBINIT" -eq 1 ]; then
	...
	if ! python "@{package.app}/manage.py" syncdb --all --noinput > "$POSTINSTALL_OUTPUT" 2>&1; then
		cat "$POSTINSTALL_OUTPUT"
		rm -f "$POSTINSTALL_OUTPUT"
	...
	if ! python "@{package.app}/manage.py" migrate forum --fake > "$POSTINSTALL_OUTPUT" 2>&1; then
		cat "$POSTINSTALL_OUTPUT"
		rm -f "$POSTINSTALL_OUTPUT"
	...
	if ! cat "@{package.app}/forum_modules/mysqlfulltext/fts_install.sql" | mysql_exec "@{osqa.db.name}" > "$POSTINSTALL_OUTPUT" 2>&1; then
		cat "$POSTINSTALL_OUTPUT"
		rm -f "$POSTINSTALL_OUTPUT"
	...
	# Apply SQL overlay
	if ! cat "@{package.app}/tmp/overlay.sql" | mysql_exec "@{osqa.db.name}" > "$POSTINSTALL_OUTPUT" 2>&1; then
		cat "$POSTINSTALL_OUTPUT"
		rm -f "$POSTINSTALL_OUTPUT"
	...
	if ! python "@{package.app}/manage.py" migrate forum > "$POSTINSTALL_OUTPUT" 2>&1; then
		cat "$POSTINSTALL_OUTPUT"
		rm -f "$POSTINSTALL_OUTPUT"
